...
import { Box, Autocomplete, TextField, Button, Typography, IconButton, Snackbar, Alert } from '@mui/material';
...
const ConsolidateAndSummary = () => {
  ...
  const validateConsolidateFields = () => {
    ...
    if (Object.keys(errors).length > 0) {
      setSnackbarMessage('Please fill in all required fields for Consolidation.');
      setSnackbarSeverity('error');
      setSnackbarOpen(true);
      return false;
    }
  ...
  const handleConsolidate = async () => {
    ...
    try {
      ...
      const consolidatedPdfBlob = new Blob([consolidatedPdfBytes], { type: 'application/pdf' });
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', marginTop: '30px', paddingTop: '100px', width: '100%', gap: '30px' }}>
        ...
        <Box
          ...
          borderRadius="10px"
          sx={{ height: 'auto', width: '30%', boxShadow: '0 4px 8px rgba(0,0,0,0.3)' }} // Adjusted width
        >
          <Typography
            variant="h4"
          ...
          />
          <Button variant="contained" color="primary" onClick={generatePDF} style={{ width: '100%', marginTop: '10px', borderRadius: '50px' }}>
            SUMMARY
          </Button>
